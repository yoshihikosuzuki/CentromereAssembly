...
def revcomp_overlap(o):
    """Revcomp a_read"""
    return Overlap(a_read_id=o.a_read_id, b_read_id=o.b_read_id, strand=o.strand,
...
def refine_mapping(ctg, tr_reads_by_id, read_id, strand, offset):
    ...
    if offset >= 0:
        aln = er_prefix.align(reverse_seq(read.seq), reverse_seq(ctg[:end_pos]))
        ...
        aln = er_prefix.align(reverse_seq(ctg[:end_pos]), reverse_seq(read.seq))
        start_pos = 0
...
def consensus_contig(ctg, edges, overlaps, tr_reads_by_id, window_size):
    ...
    mean_pos = list(filter(lambda x: x[2] + tr_reads_by_id[x[0]].length > 0, mean_pos))
    ...
    for i in range(len(ctg) // window_size):
        ...
        window_mappings = list(filter(lambda m: m[0] <= start and end <= m[1], mappings))
        window_seqs = [cut_seq(*m, start, end) for m in window_mappings]
        if len(window_seqs) == 1:
            cons += window_seqs[0]
        else:
            cons += consed.consensus(window_seqs, n_iter=3)
    return cons


def reduced_graph_to_contigs(g, overlaps, tr_reads_by_id, window_size=1000):
    out_nodes = set([e["target"] for e in g.es])
    cons_contigs = []
    for e in list(g.es):
        edges = e["edges"]
        contig = edges_to_contig(edges, tr_reads_by_id)
        logger.info(f"Edge {e['source']} -> {e['target']}: {len(contig)} bp (uncorrected)")
        cons_contig = consensus_contig(contig, edges, overlaps, tr_reads_by_id, window_size)
        logger.info(f"Edge {e['source']} -> {e['target']}: {len(cons_contig)} bp (corrected)")
        cons_contigs.append(cons_contig)
    return cons_contigs
